...
const validationFailure = (state, action) => {
  ...
  if (Array.isArray(field)) {
    ...
    validationFailures =
      validationFailed[field[0]] === false ? validationFailures + 1 : validationFailures
...
const validationSuccess = (state, action) => {
  ...
  if (Array.isArray(field)) {
    ...
    validationFailures =
      validationFailed[field[0]] === true ? validationFailures - 1 : validationFailures
    validationFailed[field[0]] = false
  } else {
    // Only devcriment faulures if this field is going from true to false
    validationFailures =
      validationFailed[field] === true ? validationFailures - 1 : validationFailures
