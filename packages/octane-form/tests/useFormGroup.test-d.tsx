...
import { useForm, useFormGroup } from '../src/index';
import type { FormGroupApi } from '../src/index';

describe('useFormGroup form-like surface', () => {
	...
	it('should infer the FormGroupApi instance type for useFormGroup', () => {
		function Comp() {
			const form = useForm({
				defaultValues: {
					step1: { name: 'test' },
...
describe('useFormGroup field-like meta surface', () => {
	...
	it('should type errors array from group validators', () => {
		function Comp() {
			...
			expectTypeOf(group.state.meta.errors).toEqualTypeOf<Array<'change-error' | undefined>>();
		}
	});
});
