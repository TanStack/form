import { DeepKeys, DeepValue } from "../util-types";

/**
 * Properly recognizes that `0` is not an object and should not have subkeys
 */
type TupleSupport = DeepKeys<{ topUsers: [User, 0, User] }>;
assertType<"topUsers" | "topUsers[0]" | "topUsers[0].name" | "topUsers[0].id" | "topUsers[0].age" | "topUsers[1]" | "topUsers[2]" | "topUsers[2].name" | "topUsers[2].id" | "topUsers[2].age">(
...
assertType<"meta" | "meta.mainUser" | "meta.mainUser.name" | "meta.mainUser.id" | "meta.mainUser.age">(
...
assertType<number>(
    0 as never as NestedArrayExample
)
