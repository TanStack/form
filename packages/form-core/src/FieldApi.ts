...
> {
  ...
  mount = () => {
    ...
    // Turns out the code below unfortunately slows down forms by a LOT, so we just give the perf back to the user if they're not using `transform`
    if (!this.form.options.transform) {
      return () => {
        // noop
      }
    }
